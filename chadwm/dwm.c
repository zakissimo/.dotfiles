...
int drawstatusbar(Monitor* m, int bh, char* stext) {
    ...
    while (text[++i]) {
        if (text[i] == '^' && !isCode) {
            ...
            text[i] = '\0';
            w = TEXTW(text) - lrpad;
            drw_text(drw, x, borderpx + vertpadbar / 2, w, bh - vertpadbar, 0,
                     text, 0);
            ...
            while (text[++i] != '^') {
                if (text[i] == 'c') {
                    ...
                    drw_rect(drw, rx + x, ry + borderpx + vertpadbar / 2, rw,
                             rh, 1, 0);
                } else if (text[i] == 'f') {
                    x += atoi(text + ++i);
                }
...
void dragcfact(const Arg* arg) {
    ...
    if (c->isfullscreen && !c->fakefullscreen) /* no support resizing fullscreen
                                                  windows by mouse */
        return;
#else
...
void dragmfact(const Arg* arg) {
    ...
    if (XGrabPointer(
            ...
            cursor[horizontal ? CurResizeVertArrow : CurResizeHorzArrow]
                ->cursor,
            CurrentTime) != GrabSuccess)
        return;
    ...
    do {
        ...
        case MotionNotify:
            ...
            if (center)
                if (horizontal)
                    if (py - ay > ah / 2)
                        fact = (double)1.0 - (ay + ah - py - ih / 2) * 2 /
                                                 (double)(ah - 2 * ih);
                    else
                        fact = (double)1.0 -
                               (py - ay - ih / 2) * 2 / (double)(ah - 2 * ih);
                else if (px - ax > aw / 2)
                    fact = (double)1.0 -
                           (ax + aw - px - iv / 2) * 2 / (double)(aw - 2 * iv);
                else
                    fact = (double)1.0 -
                           (px - ax - iv / 2) * 2 / (double)(aw - 2 * iv);
            else if (horizontal)
...
void drawbar(Monitor* m) {
    ...
    for (i = 0; i < LENGTH(tags); i++) {
        ...
        drw_setscheme(
            drw,
            scheme[occ & 1 << i ? (m->colorfultag ? tagschemes[i] : SchemeSel)
                                : SchemeTag]);
...
void drawtab(Monitor* m) {
    ...
    if (tot_width > mw) { // not enough space to display the labels, they need
                          // to be truncated
    ...
    for (c = selmon->clients; c; c = c->next) {
        ...
        if (c->isfloating || !selmon->lt[selmon->sellt]->arrange) {
            if (arg->i != 0 && prev_borderpx + arg->i >= 0)
                resize(c, c->x, c->y, c->w - (arg->i * 2), c->h - (arg->i * 2),
                       0);
            else if (arg->i != 0)
    ...
    for (i = 0; i < LENGTH(tags); i++) {
        if (selmon->tagset[selmon->seltags] & 1 << i) {
            ...
            if (occ & 1 << i && tag_preview) {
                ...
                selmon->tagmap[i] = XCreatePixmap(
                    dpy, selmon->tagwin, selmon->mw / scalepreview,
                    selmon->mh / scalepreview, DefaultDepth(dpy, screen));
                imlib_context_set_drawable(selmon->tagmap[i]);
                imlib_render_image_part_on_drawable_at_size(
                    0, 0, selmon->mw, selmon->mh, 0, 0,
...
void updatebars(void) {
    ...
    for (m = mons; m; m = m->next) {
        ...
        if (showsystray && m == systraytomon(m))
            w -= getsystraywidth();
        m->barwin =
            XCreateWindow(dpy, root, m->wx + m->gappov, m->by,
                          w - 2 * m->gappov, bh, 0, DefaultDepth(dpy, screen),
...
void updatepreview(void) {
    ...
    for (m = mons; m; m = m->next) {
        m->tagwin =
            XCreateWindow(dpy, root, m->wx, m->by + bh, m->mw / 4, m->mh / 4, 0,
                          DefaultDepth(dpy, screen), CopyFromParent,
                          DefaultVisual(dpy, screen),
                          CWOverrideRedirect | CWBackPixmap | CWEventMask, &wa);
...
void updatebarpos(Monitor* m) {
    ...
    if (m->showtab == showtab_always ||
        ((m->showtab == showtab_auto) && (nvis > 1) &&
         (m->lt[m->sellt]->arrange == monocle))) {
        m->topbar = !toptab;
        m->wh -= th + ((m->topbar == toptab && m->showbar) ? 0 : m->gappoh) -
                 m->gappoh;
        m->ty = m->toptab ? m->wy + ((m->topbar && m->showbar) ? 0 : m->gappoh)
                          : m->wy + m->wh - m->gappoh;
        if (m->toptab)
            m->wy +=
                th + ((m->topbar && m->showbar) ? 0 : m->gappoh) - m->gappoh;
    } else {
        m->ty = -th - m->gappoh;
        m->topbar = topbar;
    }
